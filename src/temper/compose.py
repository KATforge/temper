...
class Compose:
    ...
    def _run(self, path: str, *args: str, capture: bool = False) -> subprocess.CompletedProcess[str]:
        ...
        return subprocess.run(
            [executable, "compose", "-p", self.project, *(["-f", path] if path else []), *args],
    ...
    def down(self, *, volumes: bool = False):
        result = self._run("", "down", "--remove-orphans", *(["--volumes"] if volumes else []), capture=True)
        if result.returncode:
            raise state.StateError((result.stderr or result.stdout).strip() or "Compose teardown failed")
